...
class ProductController extends Controller
{
    ...
    public function createProduct(Request $request){
      ...
      if($request->hasFile('product_image')){
        ...
        $maxWidth = 1080; //new
        $maxHeight = 1080;  //new
        $full_path = $path.$filename; //new
        $image = Image::make($file->path()); //new


        // $upload = $file->move(public_path($path),$filename); // replaced by below lines

        $image->height() > $image->width() ? $maxWidth = null : $maxHeight = null; //new
        $image->resize($maxWidth, $maxHeight, function ($constraint) { //new
              $constraint->aspectRatio();
              $constraint->upsize();
        });
        $upload = $image->save($full_path); //new
        ...
        if ($saved) {
            ...
            \Log::error('Failed to save product', $product->toArray());
